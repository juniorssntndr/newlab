...
router.get('/stats', async (req, res, next) => {
    try {
        ...
        const [pedidosTotal, pedidosHoy, enProduccion, pendientes, clinicasActivas, terminadosMes] = await Promise.all([
        ...
        const recientes = await pool.query(
            `SELECT p.id, p.codigo, p.estado, p.paciente_nombre, p.fecha_entrega, p.total, c.nombre as clinica_nombre
